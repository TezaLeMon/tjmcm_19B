...
#define N_train 7	//工件总数
#define ordernumber (5+1)     //工序数
...
const int parallel_arr[ordernumber] = { N_train,parallel_a,parallel_b,parallel_c,parallel_d,parallel_e };
...
int gant(int c)		//该函数是为了将最后的结果便于清晰明朗的展示并做成甘特图，对问题的结果以及问题的解决并没有影响；
{
	int totaltime;
	int i, j;
	char machine[ordernumber * parallel][1500];
	memset(machine, '0', ordernumber * parallel * 1500 * sizeof(char));
